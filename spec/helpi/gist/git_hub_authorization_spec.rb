...
describe GitHubAuthorization do
  ...
  it "returns the access token if exists" do
    ...
    allow_any_instance_of(RestClient::Resource).to receive(:get) {'[{"token":"token", "app":{"name" : "Demo4 (API)"}}]'}
  ...
  it "returns the new access token if app not exists" do
    ...
    allow_any_instance_of(RestClient::Resource).to receive(:post) {'{"token":"new_token", "app":{"name" : "Demo4 (API)"}}'}
